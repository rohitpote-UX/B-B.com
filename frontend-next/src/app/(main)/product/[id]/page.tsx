  ...
  return (
    <div className="min-h-screen pt-40 pb-56">
      <div className="w-full max-w-[1536px] mx-auto px-8 md:px-16">
        {/* Minimal Navigation */}
        <Link href="/search" className="inline-flex items-center gap-4 text-[0.75rem] font-medium uppercase tracking-[0.15em] text-theme-muted hover:text-theme-text transition-colors mb-20">
        ...
        <div className="grid grid-cols-12 gap-8 lg:gap-16 items-center mb-48">
           ...
           <div className="col-span-12 lg:col-span-5 lg:col-start-8 py-16 lg:py-0">
              <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2 }}>
                 ...
                 <h1 className="text-[3rem] sm:text-[4.5rem] lg:text-[5.5rem] font-[var(--font-display)] font-medium leading-[1.05] tracking-tight text-theme-text mb-12">{product.name}</h1>
                 ...
                 <p className="text-[1.125rem] sm:text-[1.25rem] leading-[1.6] tracking-tight text-theme-secondary mb-20">
                   {product.description}
                 </p>
                    ...
                    {product.prices.sort((a,b) => a.price - b.price).map((p, i) => (
                       <a key={p.platform} href="#" className={`flex items-center justify-between py-6 border-b border-theme-border group ${i===0 ? 'text-theme-text' : 'text-theme-secondary hover:text-theme-text transition-colors'}`}>
                          <div className="flex items-center gap-6">
                             ...
                             <span className="text-[1.125rem] font-medium">{(PLATFORMS as any)[p.platform]?.name}</span>
                          </div>
                  ...
                  <>
                    ...
                    <div className="flex items-center gap-4 bg-theme-elevated/40 border border-theme-border/50 rounded-2xl p-4 mb-6">
                       <div className="w-16 h-16 bg-theme-elevated p-2 rounded-xl flex items-center justify-center shrink-0">
                          <img src={product.image} className="max-w-full max-h-full object-contain mix-blend-normal" alt={product.name} />
                       </div>
                       ...
                       <div className="flex flex-wrap gap-2.5">
                          {product.prices.map(p => {
                             ...
                             return (
                               ...
                               >
                                 <span>{(PLATFORMS as any)[p.platform]?.icon}</span>
                                 <span>{(PLATFORMS as any)[p.platform]?.name}</span>
                               </button>
                             )
                          })}
                       </div>
