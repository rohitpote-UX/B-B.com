...
export default function SignupPage() {
  ...
  return (
    <div className="min-h-screen flex bg-theme-bg relative z-50">
      ...
      <Link href="/" className="absolute top-8 left-8 lg:left-12 flex items-center gap-2 text-[0.75rem] font-medium uppercase tracking-[0.15em] text-theme-secondary hover:text-theme-text transition-colors z-50">
      ...
      <div className="hidden lg:flex w-[40%] bg-theme-elevated p-16 flex-col justify-between border-r border-theme-border pt-40">
         <div className="mb-24">
            ...
            >
               Join the platform that cuts through marketing noise. Get objective hardware data and unparalleled deals instantly.
            </motion.p>
         </div>
      ...
      <div className="flex-1 flex flex-col justify-center px-8 lg:px-24 xl:px-32">
         <div className="w-full max-w-md mx-auto">
            <Link href="/" className="lg:hidden text-xl font-medium tracking-tight font-[var(--font-display)] block mb-12 mt-32">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1], delay: 0.1 }}>
               <h1 className="text-[2.5rem] font-[var(--font-display)] font-medium text-theme-text mb-2 lg:hidden tracking-tight">Request Access.</h1>
               <span className="text-[0.75rem] font-medium uppercase tracking-[0.15em] block mb-12 text-theme-secondary">Establish Identity</span>

               <form onSubmit={handleSignup} className="space-y-10">
                  <div className="relative group">
                     <User className="absolute left-0 top-3 w-5 h-5 text-theme-muted group-focus-within:text-theme-text transition-colors" />
                     <input 
                        type="text" 
                        value={name}
                  ...
                  <button type="submit" className="w-full inline-flex items-center justify-center px-10 py-5 bg-theme-text border border-theme-text text-theme-bg text-[0.875rem] font-medium tracking-[0.15em] uppercase transition-transform duration-300 hover:scale-[1.02] rounded-[2px] mt-12 cursor-pointer shadow-lg shadow-black/10">
                     Initialize Account
                  </button>
               </form>

               <p className="mt-16 text-center text-[0.875rem] text-theme-secondary">
                  Already hold clearance? <Link href="/login" className="text-theme-text font-medium border-b border-transparent hover:border-theme-text transition-all duration-300 ml-2 py-0.5">Authenticate</Link>
               </p>
            </motion.div>
         </div>
      </div>
