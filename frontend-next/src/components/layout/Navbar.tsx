...
export default function Navbar() {
  ...
  useEffect(() => {
    setIsLoggedIn(!!localStorage.getItem('bb_user'))
  }, [])
    ...
    <>
      ...
      >
        <div className="w-full max-w-[1536px] mx-auto px-8 md:px-16 flex items-center  p-2 justify-between">
          {/* Logo - Minimal */}
          <Link href="/" className="text-xl font-medium tracking-tight font-[var(--font-display)]">
          ...
          <div className="hidden md:flex items-center gap-10 ">
            {links.map(link => (
              <Link
                ...
                className={`text-[0.75rem] font-medium uppercase tracking-[0.15em] transition-colors duration-400 ${
                  pathname === link.href ? 'text-theme-text' : 'text-theme-secondary hover:text-theme-text'
                }`}
              >
                {link.label}
              </Link>
            ...
            <div className="flex items-center gap-6 pl-10 border-l border-theme-border">
               {isLoggedIn ? (
                  <Link href="/profile" className="text-[0.75rem] font-medium uppercase tracking-[0.15em] text-theme-secondary hover:text-theme-text transition-colors flex items-center gap-2">
                     <User className="w-4 h-4" /> Profile
                  </Link>
               ) : (
                  <>
                     <Link href="/login" className="text-[0.75rem] font-medium uppercase tracking-[0.15em] text-theme-secondary hover:text-theme-text transition-colors">Log In</Link>
                     <Link href="/signup" className="inline-flex items-center justify-center px-6 py-2.5 bg-theme-text text-theme-bg text-[0.65rem] font-medium uppercase tracking-[0.15em] rounded-[2px] transition-transform hover:scale-[1.05]">Sign Up</Link>
                  </>
               )}
            </div>
          </div>
          ...
          <div className="md:hidden flex items-center gap-5">
            ...
            {isLoggedIn ? (
               <Link href="/profile" className="text-theme-text hover:text-theme-secondary transition-colors flex items-center justify-center w-8 h-8 rounded-full bg-theme-elevated border border-theme-border">
                  <User className="w-4 h-4" strokeWidth={1.5} />
               </Link>
            ) : (
               <Link href="/login" className="text-[0.65rem] font-medium uppercase tracking-widest text-theme-secondary border border-theme-border px-3 py-1.5 rounded-full hover:bg-theme-elevated transition-colors">
                  Log In
               </Link>
            )}
          </div>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 w-full bg-[#0a0a0a]/85 backdrop-blur-[32px] border-t border-theme-border/50 z-[100] px-8 py-4 pb-[calc(1rem+env(safe-area-inset-bottom))] flex justify-between items-center shadow-[0_-20px_40px_-20px_rgba(0,0,0,0.8)] saturate-[1.2]">
        {links.map(link => {
           ...
           return (
             <Link key={link.href} href={link.href} className={`flex flex-col items-center gap-1.5 transition-all duration-300 ${isActive ? 'text-theme-text scale-110' : 'text-theme-muted hover:text-theme-text'}`}>
