...
import {
  ...
  ChevronDown, ExternalLink, ShoppingCart, Heart, Globe
} from 'lucide-react'
import {
  AreaChart, Area, ResponsiveContainer, Tooltip, XAxis, YAxis
} from 'recharts'
import { formatPrice } from '@/data/demoData'
import { AIResponse, Product } from '@/types'
...
// ── Sentiment Mini-Bar ──
function SentimentMini({ positive, neutral, negative, label, icon: Icon, iconColor }: SentimentMiniProps) {
  return (
...
// ── Persona Fit Bar ──
function PersonaBar({ persona, delay = 0 }: PersonaBarProps) {
  return (
...
export default function AIResponseCard({ data }: { data: AIResponse }) {
  ...
  const product = topPick.product as Product
      ...
      {topPick.marketSignals.length > 0 && (
        <motion.div custom={1} variants={fadeUp} initial="hidden" animate="visible">
          <div className="flex flex-wrap gap-2">
            {topPick.marketSignals.map((signal: string, i: number) => (
              <motion.span
                key={i}
        ...
        <div className="relative bg-theme-elevated border border-[#22c55e20] rounded-2xl overflow-hidden shadow-[0_0_30px_rgba(34,197,94,0.04)]">
          ...
          <div className="p-5 sm:p-6">
            ...
            <div className="flex flex-wrap gap-1.5 mb-4">
              {product.feedBadge && <Badge text={product.feedBadge} />}
              {product.dealScore >= 85 && <Badge text="Hot Deal" />}
            </div>

            <div className="flex flex-col sm:flex-row gap-5">
              {/* Product Image */}
              <Link href={`/product/${product.id}`} className="flex-shrink-0 w-full sm:w-36 h-36 bg-white rounded-xl border border-theme-border flex items-center justify-center p-3 group">
                <img src={product.image} alt={pName} className="max-w-full max-h-full object-contain mix-blend-multiply group-hover:scale-110 transition-transform duration-500" />
              </Link>
            ...
            {topPick.dealAnalysis && (
              <div className="mt-4 pt-4 border-t border-theme-border">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
                  {topPick.dealAnalysis.platforms.map((p: any, i: number) => (
            ...
            <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-theme-border">
              <Link href={`/product/${product.id}`} className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-[#22c55e] text-black text-[0.65rem] font-bold uppercase tracking-widest hover:bg-[#16a34a] transition-colors">
                <ShoppingCart className="w-3.5 h-3.5" /> View Details
              </Link>
              <Link href={`/compare?p1=${product.id}`} className="flex items-center gap-1.5 px-4 py-2 rounded-xl border border-theme-border text-[0.65rem] font-bold uppercase tracking-widest text-theme-muted hover:text-theme-text hover:border-theme-strong transition-colors">
      ...
      <motion.div custom={3} variants={fadeUp} initial="hidden" animate="visible">
        ...
        >
          <div className="space-y-3">
            {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
            {topPick.personaFit.map((persona: any, i: number) => (
      ...
      <motion.div custom={5} variants={fadeUp} initial="hidden" animate="visible">
        ...
        >
          <div className="space-y-2 mb-4">
            ...
            {topPick.prosAndCons.pros.map((pro: string, i: number) => (
          ...
          <div className="space-y-2">
            ...
            {topPick.prosAndCons.cons.map((con: string, i: number) => (
              <motion.div key={i} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 * i }}
                className="flex items-start gap-2">
          ...
          <div className="h-40 -mx-2">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={topPick.priceHistory}>
                ...
                <Area type="monotone" dataKey="price" stroke={COLORS.green} fill="url(#priceGrad)" strokeWidth={2} dot={{ fill: COLORS.green, r: 3 }} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="bg-theme-elevated border border-theme-border rounded-2xl overflow-hidden">
            ...
            <div className="divide-y divide-theme-border">
              ...
              {alternatives.map((alt: any) => (
                <Link key={alt.product.id} href={`/product/${alt.product.id}`} className="flex items-center gap-4 px-5 py-4 hover:bg-theme-bg transition-colors group">
                  <div className="w-14 h-14 bg-white rounded-lg border border-theme-border flex items-center justify-center p-2 flex-shrink-0">
                    <img src={alt.product.image} alt={alt.product.name} className="max-w-full max-h-full object-contain mix-blend-multiply group-hover:scale-110 transition-transform duration-300" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-1.5 mb-1">
                      {alt.product.feedBadge && <Badge text={alt.product.feedBadge} />}
                    </div>
...
function CollapsibleSection({ title, subtitle, icon: Icon, iconColor, children, defaultOpen = false, id, expanded, setExpanded }: CollapsibleSectionProps) {
