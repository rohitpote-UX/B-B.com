...
export default function DealsPage() {
  ...
  const filtered = DEALS
    ...
    .sort((a, b) => sortBy === 'score' ? b.dealScore - a.dealScore : sortBy === 'discount' ? b.discount - a.discount : a.product.bestPrice - b.product.bestPrice)
      ...
      <div className="w-full max-w-[1536px] mx-auto px-8 md:px-16">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-12 mb-24 border-b border-theme-border pb-12">
          <div className="flex flex-wrap gap-8">
            {platforms.map(p => (
              ...
              >{p === 'all' ? 'ALL PLATFORMS' : PLATFORMS[p]?.name.toUpperCase()}</button>
            ))}
          </div>
        ...
        <div className="space-y-8">
          ...
          {filtered.map((deal, i) => (
            ...
            >
              <Link to={`/product/${deal.product.id}`} className="group block bg-theme-elevated hover:bg-theme-subtle transition-colors p-10 lg:p-12 rounded-sm border border-transparent hover:border-theme-border">
                 <div className="grid grid-cols-1 md:grid-cols-12 gap-12 items-center">
                    ...
                    <div className="md:col-span-2 hidden md:block">
                       <span className="text-[0.875rem] text-theme-secondary">{PLATFORMS[deal.platform]?.name}</span>
                    </div>
