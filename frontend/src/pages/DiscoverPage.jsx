...
export default function DiscoverPage() {
  ...
  const extendedFeed = useMemo(() => {
    try {
      ...
      const primary = PRODUCTS.filter(p => p.category === interest).sort(() => Math.random() - 0.5)
      const related = PRODUCTS.filter(p => mapping.rel.includes(p.category)).sort(() => Math.random() - 0.5)
      const similar = PRODUCTS.filter(p => mapping.sim.includes(p.category)).sort(() => Math.random() - 0.5)
      const trending = PRODUCTS.filter(p => p.dealScore >= 90).sort(() => Math.random() - 0.5)
      const smart = PRODUCTS.filter(p => p.rating >= 4.7).sort(() => Math.random() - 0.5)
      
      const generateChunk = () => {
         // Cycle arrays cyclically if they run low to preserve infinite scroll density
      ...
      const loop = []
      // Generate chunks but reconstruct arrays between iterations or just rely on massive generation
      for(let i=0; i<6; i++) {
         loop.push(...generateChunk())
         // Once arrays deplete, we just restart pools
         if (primary.length < 4) primary.push(...PRODUCTS.filter(p => p.category === interest).sort(() => Math.random() - 0.5))
         if (related.length < 3) related.push(...PRODUCTS.filter(p => mapping.rel.includes(p.category)).sort(() => Math.random() - 0.5))
         if (similar.length < 2) similar.push(...PRODUCTS.filter(p => mapping.sim.includes(p.category)).sort(() => Math.random() - 0.5))
         if (trending.length < 2) trending.push(...PRODUCTS.filter(p => p.dealScore >= 90).sort(() => Math.random() - 0.5))
         if (smart.length < 2) smart.push(...PRODUCTS.filter(p => p.rating >= 4.7).sort(() => Math.random() - 0.5))
      }
  ...
  const handleCompare = (product) => {
    ...
    navigate(`/compare?p1=${product.id}&p2=${peerId}`)
  }
