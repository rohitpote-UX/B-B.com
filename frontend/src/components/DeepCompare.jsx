...
import {
  ...
  CheckCircle2, XCircle, Star, BadgeCheck, ChevronDown, Wrench,
  Recycle, Smartphone, Wifi, ArrowUpRight, Sparkles, Eye,
  MessageCircle, Youtube, BookOpen, Globe, IndianRupee, DollarSign,
...
function SectionHeader({ icon: Icon, title, subtitle, index }) {
  return (
    <motion.div
      custom={index}
...
function GlassCard({ children, className = '', glow = false, winner = false }) {
  return (
    <div className={`
...
function MetricBar({ label, value, maxValue = 100, winner = false, delay = 0, icon: Icon, color = COLORS.green }) {
...
function SentimentBar({ positive, neutral, negative, label, delay = 0 }) {
  return (
...
function ProConItem({ text, isPro, delay = 0 }) {
  return (
    <motion.div
...
export default function DeepCompare({ product1, product2, winnerIndex }) {
  const [regionTab, setRegionTab] = useState('india')
  const data = useMemo(
    () => generateDeepCompareData(product1, product2, winnerIndex),
  ...
  const priceHistoryData = data.priceHistory.product1.map((p, i) => ({
    month: p.month,
        ...
        <div className="bg-theme-bg/80 backdrop-blur-xl border border-theme-border rounded-2xl px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 bg-white rounded-lg p-1 border border-theme-border">
              <img src={product1.image} alt="" className="w-full h-full object-contain mix-blend-multiply" />
            </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="w-8 h-8 bg-white rounded-lg p-1 border border-theme-border">
              <img src={product2.image} alt="" className="w-full h-full object-contain mix-blend-multiply" />
            </div>
          </div>
        </div>
      </motion.div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={priceHistoryData}>
                  ...
                  <Area
                    ...
                    fill="url(#gradP2)"
                    strokeWidth={2.5}
                    dot={{ fill: COLORS.blue, r: 4, strokeWidth: 0 }}
                    animationDuration={1500}
                  />
                  <Legend wrapperStyle={{ fontSize: '11px', fontWeight: 500, paddingTop: '12px' }} />
                </AreaChart>
            ...
            >
              <GlassCard winner={side.isWinner}>
                <div className="flex items-center gap-3 mb-6">
                  <div className="w-8 h-8 bg-white rounded-lg p-1 border border-theme-border">
                    <img src={side.product.image} alt="" className="w-full h-full object-contain mix-blend-multiply" />
                  </div>
          ...
          <div className="relative px-8 py-16 md:px-16 md:py-20">
            <div className="text-center max-w-2xl mx-auto">
              ...
              <div className="flex items-center justify-center gap-2 mt-8">
                <BadgeCheck className="w-4 h-4 text-[#22c55e]" />
                <span className="text-[0.6rem] text-theme-dim tracking-wider uppercase">
